...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "INSERT INTO quiz_questions (course_id, question_text, option_a, option_b, option_c, correct_option) 
              VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("isssss", $course_id, $question_text, $option_a, $option_b, $option_c, $correct_option);

    if ($stmt->execute()) {
        $success_message = "Quiz question added successfully!";
    } else {
        $error_message = "Failed to add quiz question.";
    }
}

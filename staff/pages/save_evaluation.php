...
$evaluation_instructions = $_POST['evaluation_instructions'] ?? '';
...
try {
    ...
    $stmt = $conn->prepare("UPDATE seminars SET evaluation_instructions = ? WHERE seminar_id = ?");
    if ($stmt) {
        $stmt->bind_param("si", $evaluation_instructions, $seminar_id);
        $stmt->execute();
        $stmt->close();
    } else {
    ...
    // Loop through each dynamic field and insert/update in evaluation_fields table
    foreach ($_POST as $key => $value) {
        if (strpos($key, 'field_') === 0) {
            ...
            $stmt = $conn->prepare("UPDATE evaluation_fields SET field_value = ? WHERE field_id = ? AND seminar_id = ?");
            if ($stmt) {
    ...
    header("Location: add_evaluation.php?seminar_id=$seminar_id&success=1");
    exit();
} catch (Exception $e) {

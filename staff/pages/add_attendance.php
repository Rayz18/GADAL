...
// Fetch attendance instructions if available
$instructions_stmt = $conn->prepare("SELECT attendance_instructions FROM seminars WHERE seminar_id = ? LIMIT 1");
$instructions_stmt->bind_param("i", $seminar_id);
$instructions_stmt->execute();
$instructions_result = $instructions_stmt->get_result();
$instructions = $instructions_result->fetch_assoc()['attendance_instructions'] ?? '';
...
<head>
    ...
    <title>Dynamic Attendance Form</title>
    <link rel="stylesheet"
...
<body class="bg-light py-5 d-flex justify-content-center">
    <div class="bg-white p-4 rounded shadow w-100" style="max-width: 800px;">
        <h1 class="text-center fs-3 fw-bold mb-4">Attendance Form</h1>
        ...
        <form id="attendance-form" action="save_attendance.php" method="POST">
            ...
            <div class="mb-3">
                <label class="form-label fw-semibold text-muted">Attendance Instructions:</label>
                <textarea class="form-control" id="instructions-textarea" name="attendance_instructions"
            ...
            <button type="submit" class="btn btn-update w-100" id="update-button">Update Attendance</button>
        </form>
    </div>
    ...
    <script>
        ...
        function submitField() {
            ...
            fetch('add_field_attendance.php', {
                method: 'POST',
                headers: {
        ...
        function deleteField(fieldId) {
            ...
            if (confirm("Are you sure you want to delete this field?")) {
                fetch('delete_field_attendance.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
